...
pub fn psd_from_image_bytes(name: &str, bytes: &[u8]) -> Result<Vec<u8>, String> {
    let img = image::load_from_memory(bytes)
        .map_err(|e| format!("Failed to decode image: {e}"))?;
    let (width, height) = img.dimensions();
    psd_from_rgba(name, width, height, img.to_rgba8().into_raw())
}
...
pub fn import_file_as_psd(
    ...
    if ext == "psd" {
        ...
        // than halfway through the pipeline.
        psd::Psd::from_bytes(&bytes).map_err(|e| format!("Not a readable PSD: {e}"))?;
        std::fs::write(&dest, &bytes).map_err(|e| e.to_string())?;
    } else {
        let psd_bytes = psd_from_image_bytes(&stem, &bytes)?;
