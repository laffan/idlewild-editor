...
- Apply writes the extrusion as a PSD, marked and anchored on the spaces it
  ...
  it. The shape outlives every edit to the file — a re-parse, a re-import, a
  layer stack rewritten in the inspector — so the way back in is always there.
  And applying again **rewrites** the file rather than replacing it: the
  artwork layer and both marks come out regenerated, each back where it was in
  the stack, and a layer painted over the greybox in Photoshop is still there
  afterwards, still lined up on the artwork
